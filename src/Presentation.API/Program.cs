...
app.MapPost("/organizers", async ([FromBody] CreateOrganizerCommand command, ISender sender, CancellationToken ct) =>
{
    var organizerCreatedId = await sender.Send(command, ct);
    return Results.Ok(organizerCreatedId);
});

app.MapPost("/organizers/{id:guid}/tournaments", async (Guid id, string tournamentName, ISender sender, CancellationToken ct) =>
{
...
app.MapPost("organizers/{id:guid}/tournaments/teams/{teamId:guid}", async (Guid id, Guid teamId, ISender sender, CancellationToken ct) =>
{
    await sender.Send(new RegisterTeamCommand { OrganizerId = id, TeamId = teamId }, ct);
    return Results.NoContent();
});

app.MapDelete("/organizers/{id:guid}/tournaments/teams/{teamId:guid}", async (Guid id, Guid teamId, ISender sender, CancellationToken ct) =>
{
    await sender.Send(new DiscardTeamCommand { TeamId = teamId, OrganizerId = id }, ct);
    return Results.NoContent();
});

app.MapPost("/organizers/{id:guid}/tournaments/matches", async (Guid id, ISender sender, CancellationToken ct) =>
{
    await sender.Send(new MatchTeamsCommand { OrganizerId = id }, ct);
    return Results.NoContent();
});

app.MapDelete("/organizers/{id:guid}/tournaments", async (Guid id, ISender sender, CancellationToken ct) =>
{
    await sender.Send(new FinishTournamentCommand { OrganizerId = id }, ct);
    return Results.NoContent();
});

app.MapGet("/organizers/{id:guid}", async (Guid id, ISender sender, CancellationToken ct) =>
{
    var organizer = await sender.Send(new GetOrganizerQuery { OrganizerId = id }, ct);
    return Results.Ok(organizer);
});

app.MapGet("/organizers/{id:guid}/tournaments/matches", async (Guid id, ISender sender, CancellationToken ct) =>
{
